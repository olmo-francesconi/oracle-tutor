...
api/
  src/oracle_tutor_api/
    ...
    semantic/           model training, embedding computation, runtime semantic search
    worker/             one-shot Scryfall ingestion and Tagger sync
...
## Components
- `api/src/oracle_tutor_api/api/main.py`: single FastAPI app with lifespan-driven DB init and routes for `/search`, `/suggest-names`, `/card/{id}`, `/search-oracle`, `/similar-cards/{id}`, `/health`, `/version`.
- `api/src/oracle_tutor_api/api/schemas.py`: Pydantic response models; `SimilarCard` is the shared payload for semantic search and similarity results.
- `api/src/oracle_tutor_api/core/database.py`: builds SQLAlchemy engine from `DATABASE_URL` or local `DB_*` vars, treats Railway as production, uses `StaticPool` for in-memory SQLite tests.
- `api/src/oracle_tutor_api/core/db_init.py`: owns schema bootstrap, Postgres extension/index creation, migration-state coordination via `system_metadata`, and destructive reset gating for worker mode.
- `api/src/oracle_tutor_api/core/models.py`: ORM schema for cards, faces, ingestion logs, tags, relationships, and `card_face_semantic_embeddings`; embeddings are face-level with dimension 384.
- `api/src/oracle_tutor_api/worker/main.py`: cron-friendly one-shot entry point; runs ingestion once and exits.
- `api/src/oracle_tutor_api/worker/data_builder.py`: downloads Scryfall bulk data, filters non-playable/digital-only records, chooses a preferred printing per `oracle_id`, upserts cards/faces, cleans stale rows, and optionally triggers tag sync.
- `api/src/oracle_tutor_api/worker/fetch_tags.py`: pulls GraphQL data from `tagger.scryfall.com` and stores tags, ancestor links, and card relationships.
- `api/src/oracle_tutor_api/semantic/train.py`: trains a sentence-transformer model from self-pairs plus tag-derived positive pairs and saves it to `data/semantic/model`.
- `api/src/oracle_tutor_api/semantic/compute.py`: recomputes all face embeddings offline and stores them in Postgres.
- `api/src/oracle_tutor_api/semantic/index.py`: lazy-loads the trained model at runtime and executes pgvector cosine-distance queries.
...
## Data flow
1. The ingestion worker initializes schema in worker mode, acquiring a Postgres advisory lock and marking migration state in `system_metadata`.
2. `worker/data_builder.py` fetches Scryfall bulk metadata and the default-cards file, filters out unsupported records, reduces multiple printings to one preferred printing per `oracle_id`, and upserts `cards` plus `card_faces`.
...
4. Semantic model training and embedding computation are offline jobs. Training writes model assets to `api/data/semantic/model`; compute writes vectors to `card_face_semantic_embeddings`.
...
## Key conventions
- Python code uses a `src/` layout rooted at `api/src/oracle_tutor_api`; tests add that path manually instead of relying on editable installs.
- Database schema is owned directly in SQLAlchemy models plus `core/db_init.py`; there is no Alembic migration layer.
- `init_db()` has two modes: API mode must never perform destructive resets, worker mode may reset card tables only when `ORACLE_TUTOR_API_ALLOW_SCHEMA_RESET=true`.
...
- Semantic assets live under `api/data/semantic/model`; request-time inference assumes those files already exist.
...
- Versioning is intentionally mirrored: `api/pyproject.toml` and `frontend/package.json` are both `1.4.0`.

## How to run
- Dev: `docker compose up --build`
- Backend setup: `cd api && uv sync --all-extras --group test`
- Backend checks: `cd api && uv run ruff check src/ --fix && uv run basedpyright && uv run pytest tests/ -x -q`
- Frontend dev: `cd frontend && npm run dev`
- Frontend checks: `cd frontend && npm run lint && npm run build`
- Build: `docker compose build`
- Worker only: `docker compose -f docker-compose.worker.yml up --build worker`
...
- The codebase prefers direct, explicit modules over deep abstraction layers; most behavior is concentrated in a few large files (`api/main.py`, `worker/data_builder.py`, `components/CardGrid.tsx`, `components/FilterBar.tsx`).
...
- The semantic worker exists in local compose and has a dedicated Dockerfile, but `docker-compose.prod.yml` does not currently define that service.
- `docker-compose.worker.yml` still contains legacy TF-IDF callback environment variables; those are stale relative to the semantic-only backend direction and should not be treated as current architecture.
...
- Planned rename work appears in `.agent-config/PINBOARD.md`, but the checked-in code still uses `oracle_tutor_api`, `worker`, and `semantic`.
