...
- Tag-derived training pairs carry a **mined hard negative** (`build_hard_negative_pools`): a face from a tag that co-occurs with T, ranked by *association* (`overlap / size of the other tag`) rather than raw overlap, so generic tags like `activated ability` (8.8k cards) do not dominate. Without it MultipleNegativesRankingLoss sees only ~31 random in-batch negatives and nothing teaches a concept's boundary. For `mana dork` the mined negatives are mana sources that are lands or artifacts — precisely the contrast that defines it
