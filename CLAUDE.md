...
- **Training is ability-granular**, matching what is served: nothing at runtime embeds a face, so `TrainingDatasetState` is keyed by `(oracle_id, face_ix, ability_ix)` and every positive, negative and self-pair is one ability's `normalized_text`. Positives are deduplicated by text within a tag — 63,289 ability rows collapse to 37,339 distinct, and `Flying.` alone is 3,235 of them, so without dedup a keyword tag spends its whole budget on one string
- `semantic/tag_attribution.py` — tags live on **cards**, abilities two layers below, so a tag must be attached to the ability that justifies it. Pairing `mana dork` with the whole of Birds of Paradise taught the model it also means `Flying`. Attribution scores each ability by **within-tag token lift** (how much its tokens recur across the tag's members relative to corpus-wide frequency) and keeps the winner only if it beats the runner-up by `DEFAULT_MIN_MARGIN`. The signal is deliberately model-free: using embedding similarity would ask the model which ability it already believes matches, and `mana dork` was broken in exactly that way. The margin is calibrated on hand-labelled samples (~86% precision at 0.5 vs ~64% at 0.3), and a second pass drops any tag resolving under half its multi-ability members — that is what removes `alliteration`, `type addition human` and other name/type-line properties no ability can justify (741 of 4,707 tags)
- Tag-derived training pairs carry a **mined hard negative** (`build_hard_negative_pools`): an ability from a tag that co-occurs with T, ranked by *association* (`overlap / size of the other tag`) rather than raw overlap, so generic tags like `activated ability` (8.8k cards) do not dominate. Without it MultipleNegativesRankingLoss sees only ~31 random in-batch negatives and nothing teaches a concept's boundary. For `mana dork` the mined negatives are the *mana abilities of lands* — precisely the contrast that defines it, and sharper than the whole-land text it used to be
- `dataset_service.build_state_from_maps` is the single definition of a training example. `modal_train._build_dataset_state` parses the shipped payload and calls it rather than keeping a second copy, which had drifted before. Build payload is **v3** (abilities + attributed `tag_to_ability_ids`, built locally so attribution runs once); dataset format is **v8**, and v2–v7 still load by reading each face as a single unit
- `semantic/text_prep.py` — `normalize_oracle_text`; the self-reference is **type-aware** everywhere, including `{T}` → "tap this creature" / "tap this land". Rendering `{T}` generically collapsed a Forest and Llanowar Elves onto one vector, making "a creature that taps for mana" unrepresentable. Costs only ~135 extra distinct ability texts (37,204 → 37,339). Changing normalization changes `text_hash`, so it takes effect on the next ingest + promote
...
- `ingest/scryfall_ingestion.py` — **`--skip-tags` preserves `card_taggings`** (`preserve_taggings`): a card refresh clears its dependent rows, and the Tagger phase normally rebuilds them, so skipping that phase without this deletes the tag corpus outright and nothing puts it back. Cards being *deleted* still drop theirs. Note the corollary: a normal (non-skip) ingest still clears taggings and refetches all ~33.6k cards from Tagger, which is the ~9h tail on every full run. — stale-aware Scryfall bulk ingest (gzipped JSON Lines via `jsonl_download_uri`), card derivation, `type_categories` extraction, ability segmentation; delete phase is a single transaction (no split-state on crash)
